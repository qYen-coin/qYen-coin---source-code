...
- [Development guidelines](#development-guidelines)
    - [General qYen Core](#general-qyen-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
qYen Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/qyenpay/qyen/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_qyen
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_qyen --log_level=test_suite
$ valgrind -v --leak-check=full src/qyend -printtoconsole
```
...
# A coverage report will now be accessible at `./test_qyen.coverage/index.html`.
```

**Sanitizers**

qYen Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 7368.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 7367 for connections and services them.
...
However, in open source software such as qYen Core, where everyone uses
...
pay attention to for reviewers of qYen Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: qYen Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against qYen Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `qyen-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `qyen-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of qYen Core, the wallet was always
